...
export interface PowerSchoolBellWorkerOutput {
...
export type PowerSchoolBellSourceFactory = (options: {
  readonly config: PowerSchoolRoutineConfig;
  readonly environment: NodeJS.ProcessEnv;
  readonly signal?: AbortSignal;
}) => ScheduleObservationSource;

export async function runPowerSchoolBellWorker(options: {
  readonly arguments: readonly string[];
  readonly environment?: NodeJS.ProcessEnv;
  readonly signal?: AbortSignal;
  readonly sourceFactory?: PowerSchoolBellSourceFactory;
  ...
  let config: PowerSchoolRoutineConfig;
  try {
  ...
  try {
    const source =
      ...
      new PassivePowerSchoolBellScheduleSource(config, {
        environment,
        ...(options.signal === undefined ? {} : { signal: options.signal }),
      });
    ...
    return {
      ...
      result: {
        ...
        error: {
          ...
          message:
            'PowerSchool collection failed before producing an observation.',
          retryable: false,
          diagnostics: [],
        },
      },
...
async function main(): Promise<void> {
  ...
  const output = await runPowerSchoolBellWorker({
