...
gate passes. Redistributable media and reachable history are resolved by the
poster fallback, external-media contract, and curated-root procedure; the exact
published commit must still pass GitHub CI before a visibility change.
...
The original migration history audit reported historical personal paths,
private evidence location references, and a non-noreply author identity. That
history remains private in a separately named archive. The public candidate is
the separately reviewed, one-commit curated root described in
[the public history plan](publication-history-plan.md), not an in-place rewrite
